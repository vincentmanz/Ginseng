...
sudo chown $USER *vcf.gz
sudo chown $USER *vcf.gz.tbi
...
mkdir -p ./vcftools
VCF=test_deepvariant.cohort.vcf.gz
OUT=./vcftools/test_deepvariant.cohort
...
VCF=../test_deepvariant.cohort.vcf.gz
VCF_OUT=test_deepvariant.cohort_filtered.vcf.gz
# set filters
MAF=0.05
MISS=0.8
QUAL=30
...
VCF=test_deepvariant.cohort_filtered.vcf.gz
OUT=./vcftools/test_deepvariant.cohort_filtered_out.vcf.gz
#Calculate allele frequency
vcftools --gzvcf $VCF --freq2 --out $OUT --max-alleles 2
#mean depth of coverage per individual
vcftools --gzvcf $VCF --depth --out $OUT
